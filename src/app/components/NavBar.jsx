      ...
      <div className="flex-row justify-between items-center bg-gray-100 p-2 hidden sm:flex cursor-pointer">
        ...
        <div className="flex flex-row items-center">
          <ul className="flex justify-center items-center gap-2 text-xl">
            <li>
              <a
                className="w-[38px] h-[38px] hover:text-blue-600 flex justify-center items-center"
                href="#"
              >
                <FaFacebookF />
              </a>
          ...
          <button
            className={`nav-button rounded-lg ml-5 bg-[#366cff] w-[110px] h-[45px] ${
              isNavVisible ? "text-black hidden bg-white" : "text-white block"
            }`}
      ...
      <div className="nav-container flex justify-between m-3 items-center">
        <div className="flex items-center">
          <CiMenuBurger
            ...
            onClick={toggleNav}
          />
          <Image src={logo} className="w-[150px] h-[80px] ml-2" />
        </div>
        <div
          className={`nav-right  space-x-4 text-lg flex flex-col md:flex-row items-start md:items-center space-y-4 md:space-y-0 md:space-x-8 font-poppins ${
            isNavVisible
              ? "fixed top-0 left-0 w-full md:w-[1000px] bg-[#366cff] h-screen md:h-[300px] justify-start items-start text-white space-x-8 space-y-6 z-50 p-5"
              : "hidden"
          } md:flex`}
        >
          <button
            className="nav-button bg-white text-gray-600 rounded-full font-bold w-[50px] h-[45px] absolute top-5 right-5 md:hidden"
            onClick={toggleNav}
          >
            {" "}
            X{" "}
          ...
          <button
            className="nav-button bg-white text-gray-600 rounded-lg font-bold w-[110px] h-[45px] md:hidden"
            onClick={handleClickLogin}
          >
            Log In
          </button>
